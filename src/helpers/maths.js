...
export const pickWithProbabilities = (entries, random = 0, providedSum) => {
  ...
  console.error({ entries, random, providedSum, sum });
};
...
export const reflect = (vector, normal, out = vector) => {
  var d = vector[0] * normal[0] + vector[1] * normal[1];
  out[0] = vector[0] - 2.0 * d * normal[0];
  out[1] = vector[1] - 2.0 * d * normal[1];
  return out;
};

export const mix = (from, to, time = 0.5, out = []) => {
  if (Array.isArray(from) || ArrayBuffer.isView(from)) {
    for (let index = 0; index < from.length; index++) {
      out[index] = from[index] * (1 - time) + to[index] * time;
    }
    return out;
  } else {
...
export function shuffleArray(array, seed) {
  const randomizer = seed === undefined ? Math.random : createRNG(seed);
...
export const unproject = (x, y, z, matrix, out = [0, 0, 0]) => {
